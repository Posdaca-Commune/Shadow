...
Windows 发布资产以 MSIX 包形式发布。包内包含主程序和内置插件，用户不需要
手动解压 zip，也不需要额外安装 .NET 运行时。

## 快速开始

1. 从 GitHub Release 下载 `Shadow-1.0.0-beta.1.msix`。
2. 使用 Windows 应用安装程序安装该包。
...
Shadow 可以在启动桌面 UI 之前分发插件命令。MSIX 安装包会注册 `shadow.exe`
应用执行别名，因此可以在终端里直接使用 `shadow` 调用内置 HOI4 插件命令：
...
创建包含内置插件的 Windows x64 MSIX 包前，先安装 Windows 10/11 SDK，然后
运行：
...
生成的包位于 `artifacts/msix/Shadow-1.0.0-beta.1.msix`。MSIX 安装前必须签名；
可通过 `-CertificatePath` 或 `-CertificateThumbprint` 在打包时签名。
