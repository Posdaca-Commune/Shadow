...
> 状态：`1.1.3` 是当前版本。可用于 Windows 常规使用和商店打包，内置启动器
> 插件已有初步 Linux 支持。在把它作为唯一启动器之前，仍建议先备份游戏用户
> 配置目录。
...
- 存档管理：浏览、搜索和删除存档，显示游戏内日期和国家元数据；
  在资源管理器中定位存档；群星的文件夹型存档显示为单个卡片并标注文件数。
- 支持命令行插件命令分发，当前包含 `paradox.launch`。

## 运行要求

- Windows 10 或 Windows 11（MSIX 包）。CI 构建的 Linux 便携 tar 包（x64/arm64）
  也可运行壳层和内置启动器插件，支持原生 Paradox 游戏安装或 Steam。
...
1. 从 GitHub Release 下载 `Shadow-1.1.3.msix`。
...
生成的包会写入 `artifacts/msix/Shadow-1.1.3.msix`。
